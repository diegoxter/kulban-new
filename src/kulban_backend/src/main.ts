...
app.post(
  ...
  async (req: Request, res: Response) => {
    ...
    try {
      await createCategory(boardAddress, newCategory);
...
app.delete(
  ...
  async (req: Request, res: Response) => {
    ...
    try {
      await removeTask(boardAddress, taskID);
...
app.patch(
  ...
  async (req: Request, res: Response) => {
    ...
    try {
      await editCategory(boardAddress, categoryData);
...
app.patch(
  ...
  async (req: Request, res: Response) => {
    const {
      ...
      taskIDs,
      tasks,
    }: { boardAddress: string; taskIDs: number[]; tasks: Omit<Task[], "id"> } =
      req.body;

    try {
      await editTasks(boardAddress, taskIDs, tasks);
